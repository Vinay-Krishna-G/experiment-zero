...
export default function LedgerCard({ event }: { event: LedgerEvent }) {
  ...
  const severityColors = {
    ...
    info: "border-[var(--border-subtle)] bg-[var(--bg-card)] text-[var(--fg-secondary)]",
  };
  ...
  return (
    <article 
      className={`p-6 rounded-lg border transition duration-300 hover:border-white/15 hover:bg-[var(--bg-card-hover)] ${severityColors[severity]}`}
      aria-label={`${title} event on ${date}`}
    >
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-3 border-b border-[var(--border-subtle)]">
        <div className="flex items-center gap-3">
          <span className={`px-2.5 py-0.5 rounded text-[0.6rem] font-mono tracking-widest uppercase border ${badgeColors[type]}`}>
            {type.replace("_", " ")}
          </span>
          <span className="text-[0.65rem] font-mono tracking-widest text-[var(--fg-muted)]">
            {date}
          </span>
        </div>
      ...
      <div className="mb-4">
        <h4 className="font-serif text-lg text-[var(--fg-primary)] font-medium tracking-wide mb-2">
          <Link href={`/${route}/${slug}`} className="hover:text-emerald-300 transition">
            {title}
          </Link>
        </h4>
        <p className="text-sm text-[var(--fg-secondary)] font-light leading-relaxed">
          {description}
        </p>
      </div>

      {aiSummary && (
        <div className="mb-4 p-3 rounded bg-[var(--bg-card)] border border-[var(--border-subtle)] text-xs text-[var(--fg-muted)] italic leading-relaxed">
          {aiSummary}
        </div>
      )}

      {/* Semantic relation panel (Narrative Graph links) */}
      <footer className="flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-[var(--border-subtle)]">
        {semanticTags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 items-center">
            <span className="text-[0.55rem] font-mono uppercase tracking-widest text-[var(--fg-subtle)] mr-1">Tags:</span>
            {semanticTags.map((tag) => (
              <Link
                key={tag}
                href={`/ledger/tag/${tag.toLowerCase()}`}
                className="px-2 py-0.5 text-[0.6rem] font-mono bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:border-white/25 hover:bg-white/10 rounded text-[var(--fg-secondary)] hover:text-[var(--fg-primary)] transition"
              >
                #{tag}
              </Link>
            ))}
        ...
        {relatedTriples && relatedTriples.length > 0 && (
          <div className="text-[0.58rem] font-mono text-[var(--fg-subtle)]">
